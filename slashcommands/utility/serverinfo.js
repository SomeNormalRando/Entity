...
module.exports = {
	...
	async execute(interaction) {
		...
		guildFeatures.forEach((element, index) => {
			guildFeatures[index] = normalizeStr(element);
		});
		let afkTimeout = '';
		if (guild.afkChannel) {
		...
		const embed = new Discord.MessageEmbed()
			...
			.addFields(
				...
				`, inline: true },
				{ name: 'Moderation', value: `Verification Level: ${normalizeStr(guild.verificationLevel)}\nExplicit Content Filter: ${normalizeStr(guild.explicitContentFilter)}` },
